...
defaultLocation = "/root/hs-count-imports/output"

default_opts :: Opts
default_opts = Opts { inputDir       = defaultLocation
                    , quiet          = False
                    , with_missing   = False
                    , use_clusters   = True
                    , mod_in_cluster = True
...
add_ignore_qual s o = o
    { ignore_mods = Trie.insert (splitQualifier s)
                                (const IgnoreAll)
                                (ignore_mods o)
    }

add_color_scheme :: String -> OptT
add_color_scheme n o = o
    { color_scheme = case reads n of
                         [(x, "")] -> x
                         _         -> color_scheme default_opts
    }

add_collapse_qual :: Bool -> String -> OptT
add_collapse_qual m s o = o
    { collapse_quals = upd (splitQualifier s) (collapse_quals o)
    }
