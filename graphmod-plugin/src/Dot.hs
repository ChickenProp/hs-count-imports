...
noEdges :: AllEdges
noEdges = AllEdges { normalEdges = IMap.empty, sourceEdges = IMap.empty }
...
pruneEdges es = foldr checkEdges es (IMap.toList es)
  where
    ...
    reachIn g tgt visited (x : xs)
        | x `ISet.member` visited
        = reachIn g tgt visited xs
        | x == tgt
        = True
        | otherwise
        = let vs = neighbours g x
...
collapse opts (Trie.Sub ts mb) ([q], alsoMod) = do
    t <- Map.lookup q ts
    ...
    let close ((_, nm), _) =
            ((if will_move then Deleted else Redirect, nm), rep)
  ...
  where
    ...
    findThisMod (x : more)                        = do
        (yes, more') <- findThisMod more
        return (yes, x : more')
    findThisMod [] = Nothing
...
collapse opts (Trie.Sub ts ms) (q : qs, x) = do
    t  <- Map.lookup q ts
...
moveModulesInCluster (Trie.Sub su0 ms0) = goMb
    (fmap moveModulesInCluster su0)
    ms0
  where
    goMb su mb = case mb of
        Nothing -> Trie.Sub su Nothing
        Just xs -> go [] su xs

    go ns su xs = case xs of
        []     -> Trie.Sub su $ if null ns then Nothing else Just ns
        y : ys -> case check y su of
            Left  it  -> go (it : ns) su ys
            Right su1 -> go ns su1 ys

    check it@((nt, s), i) mps = case nt of
        ModuleNode -> case Map.lookup s mps of
            Nothing -> Left it
...
make_dot opts (es, t) = showDot $ do
    attribute ("size", graph_size opts)
  ...
  where
    ...
    genEdges attr edges = forM_ (IMap.toList edges) $ \(x, ys) ->
        forM_ (ISet.toList ys)
            $ \y -> edge (userNodeId x) (userNodeId y) (attr x y)
...
make_clustered_dot cs0 su = go (0, 0, 0) cs0 su >> return ()
  where
    ...
    go outer_col ~(this_col : more) (Trie.Sub xs ys) = do
        let outerC = renderColor outer_col
        ...
        forM_ (fromMaybe [] ys) $ \((t, ls), n) ->
            unless (t == Redirect || t == Deleted)
                ...
                ++ case t of
                       ...
                       CollapsedNode True ->
                           [("style", "filled"), ("fillcolor", clusterC)]
                       ModuleInItsCluster ->
                           [("style", "filled,bold"), ("fillcolor", outerC)]
    ...
    goSub outer_col cs ((name, sub) : more) = do
        (_, cs1) <- cluster $ do
...
make_unclustered_dot c pre (Trie.Sub xs ys') = do
    let col = renderColor (head c)
    let ys  = fromMaybe [] ys'
    forM_ ys $ \((t, ls), n) ->
        userNode (userNodeId n)
            $  [("fillcolor", col), ("style", "filled"), ("label", pre ++ ls)]
            ++ case t of
...
renderColor (x, y, z) = '#'
    : showHex (mk x) (showHex (mk y) (showHex (mk z) ""))
